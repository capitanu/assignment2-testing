...
public class Point {
    ...
    // For a point to be in the circle,
    // x and y must satisfy (Point.x-Center.x)^2 + (Point.y-Center.y)^2 < radius^2
    ...
    public static Circle findSmallestCircle2Points(Point p0, Point p1) {
        double radius = Math.pow(calculateDistance(p0, p1) / 2, 2);
    ...
    public static double calcRadiusSmallestCircle(Point p0, Point p1, Point p2) {
        ...
        if (pointIsInCircle(p1, circle2)) {
            radius2 = circle1.getRadius();
        }
        if (pointIsInCircle(p0, circle3)) {
            radius3 = circle1.getRadius();
        }
